...
export class ProfilAdminComponent implements OnInit {
  ...
  modifier(form: FormGroup){
    this.userService.modifieruser(this.user.id, form.value).subscribe(
      ...
      error => {this.tosterService.success("modification terminée");
  ...
  updateUser() {
    ...
    this.userService.modifierPhoto(formData , this.user.id)
      .subscribe(res => {
        this.tosterService.success('modification terminée !!', 'modification');
          window.location.reload();
      }
    ,erreur=>{this.tosterService.success('modification terminée !!', 'modification');
          window.location.reload();}
      );
  }
